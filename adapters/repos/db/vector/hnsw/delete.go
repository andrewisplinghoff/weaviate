...
func (h *hnsw) cleanUpTombstonedNodes(shouldAbort cyclemanager.ShouldAbortCallback) (bool, error) {
	...
	h.logger.Debugf("before copyTombstonesToAllowList\n")
	...
	h.logger.Debugf("before removeTombstonesAndNodes\n")
			...
			for {
				...
				case deletedID, ok := <-ch:
					...
					if h.getEntrypoint() != deletedID {
						h.logger.Debugf("before reassignNeighbor call in reassignNeighborsOf deletedID=%d\n", deletedID)
						...
						h.logger.Debugf("after reassignNeighbor call in reassignNeighborsOf deletedID=%d\n", deletedID)
					}
				}
			}
		})
